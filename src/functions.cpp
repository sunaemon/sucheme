...
namespace sucheme{
    ...
    {
        env_intern_define(e, "+", alloc<Procedure>(sucheme::add));
        env_intern_define(e, "=", alloc<Procedure>(sucheme::eq));
        env_intern_define(e, "*", alloc<Procedure>(sucheme::mul));
        env_intern_define(e, "-", alloc<Procedure>(sucheme::sub));
        env_intern_define(e, "car", alloc<Procedure>(sucheme::car));
        env_intern_define(e, "cdr", alloc<Procedure>(sucheme::cdr));
        env_intern_define(e, "print", alloc<Procedure>(sucheme::print));
        env_intern_define(e, "null?", alloc<Procedure>(sucheme::null_is));
        env_intern_define(e, "else", alloc<Bool>(true));
   ...
   GCObject* null_is(const vector<GCObject* > &arg) {
       ...
       return alloc<Bool>(dcast<Empty>(arg[0]));
    }
}
